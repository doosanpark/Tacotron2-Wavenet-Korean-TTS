...
def build_from_path(hparams, in_dir, out_dir, num_workers=1, tqdm=lambda x: x):
    """
    Preprocesses the KSS dataset from a given input path to given output directories

    Args:
        - hparams: hyper parameters
        - in_dir: input directory that contains the files to preprocess
    ...
    # KSS 데이터셋의 transcript 파일 경로
    transcript_path = os.path.join(in_dir, 'transcript.v.1.4.txt')

    with open(transcript_path, encoding='utf-8') as f:
        for line in f:
            parts = line.strip().split('|')
            if len(parts) >= 4:
                # KSS 형식: wav_path|text1|text2|text3|duration|english
                # text2 (parts[2])를 사용
                wav_file = parts[0]  # e.g., "1/1_0000.wav"
                text = parts[2]  # 두 번째 텍스트 사용

                # wav 파일의 전체 경로 생성
                wav_path = os.path.join(in_dir, 'kss', wav_file)
...
def _process_utterance(out_dir, wav_path, text, hparams):
    ...
    Args:
        - out_dir: the directory to write the output files into
        - wav_path: path to the audio file containing the speech input
        - text: text spoken in the input audio file
        - hparams: hyper parameters

    Returns:
        - A tuple: (audio_filename, mel_filename, linear_filename, time_steps, mel_frames, text, npz_filename)
    """
    try:
        # Load the audio as numpy array
        wav = audio.load_wav(wav_path, sr=hparams.sample_rate)
    except FileNotFoundError: # catch missing wav exception
        print('file {} present in transcript is not present in wav folder. skipping!'.format(wav_path))
        return None
